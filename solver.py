...
class BoardState:
    board: Board
    steps: list[Step] = _EMPTY_LIST_FIELD
    ...
    def set_tile(self, i: int, j: int, value: int, is_trivial_step: bool) -> 'BoardState':
        if self.get_tile(i, j):
            raise ValueError(f"Cannot set ({i}, {j}) as there is already a tile there. board=\n{self.board}")
        new_grid = deepcopy(self.board.grid)
        new_grid[i][j] = value
        step = Step(i, j, value, is_trivial_step)
        return BoardState(
            Board(new_grid),
            [*self.steps, step]
        )
...
class Solver:
    ...
    def get_candidates(self, state: BoardState) -> CandidateBoard:
        ...
        for (i, j) in self.indexer.all_coords():
            ...
            if tile_value:  # (is not blank)
                candidates[i][j] = set()
                for ii, jj in self.indexer.get_all_containing_sequences(i, j):
                    st = candidates[ii][jj]
                    if tile_value in st:
                        st.remove(tile_value)
            ...
            for i, j in sequence:
                if not state.get_tile(i, j):
                    ...
                    if len(diff) == 1:
                        ...
                        raise InvalidBoardException(f"According to candidate analysis, tile ({i}, {j}) must take on more than one value: {diff}. This board has no solution.")
        return state

    def has_valid_child_states(self, candidates: CandidateBoard, state: BoardState) -> bool:
    ...
    def resolve_trivial_constraints(self, state: BoardState) -> BoardState:
        ...
        while updated_state != prev_state:
          ...
          if not self.has_valid_child_states(candidates, prev_state):
              raise InvalidBoardException("Tile(s) found which have no candidates.")
    ...
    def get_child_states(self, state: BoardState) -> list[BoardState]:
    ...
    def solve(
            ...
            method: SearchMethod = SearchMethod.DFS) -> SolverResult:
        ...
        while queue:
            ...
            for next_state in self.get_child_states(updated_state):
